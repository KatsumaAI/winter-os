...
function buildUserPayload(user = {}) {
    return {
        ...
        avatar_decoration: user.avatar_decoration || 'none',
        ...
        featured_item_ids: Array.isArray(user.featured_item_ids) ? user.featured_item_ids : [],
...
function attachUserSecurityMeta(user, req) {
    ...
    user.login_history = safeArray(user.login_history);
    const agent = normalizeText(req.headers['user-agent'] || '').slice(0, 220);
    user.login_history.unshift({
        at: user.last_login_at,
        ip,
        user_agent: agent
    });
    user.login_history = user.login_history.slice(0, 18);
}
