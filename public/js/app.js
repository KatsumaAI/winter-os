(function(){
  ...
  function renderAgenda(){
    ...
    const all = readLS('agendaItems', []);
    const items = all.filter(x => x && x.date === sel);
    if (!items.length){
      list.innerHTML = '<div class="subtitle">No items yet.</div>';
      return;
    }

    list.innerHTML = items.map(it => (
      `<div class="aitem"><div class="at">${String(it.text || '').replace(/[&<>"']/g, (c) => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;','\'':'&#39;'}[c]))}</div><button class="ax" data-del="${it.id}" aria-label="Remove"><i data-feather="x"></i></button></div>`
    )).join('');
    ...
    qsa('[data-del]', list).forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const next = all.filter(x => x && x.id !== id);
        writeLS('agendaItems', next);
        renderAgenda();
      });
    });
  }
  ...
  function addAgenda(){
    const input = byId('agendaInput');
    if (!input) return;
    const text = input.value.trim();
    if (!text) return;
    const all = readLS('agendaItems', []);
    all.unshift({ id: String(Date.now()) + Math.random().toString(16).slice(2), date: sel, text });
    writeLS('agendaItems', all);
    input.value = '';
    renderAgenda();
  }
  ...
  function renderPosts(){
    ...
    if (!list) return;
    const posts = readLS('socialPosts', []);
    if (!posts.length){
      list.innerHTML = '<div class="subtitle">No posts yet. Drop your first update above.</div>';
      return;
    }
    list.innerHTML = posts.slice(0, 25).map(p => {
      const dt = new Date(p.ts);
      const when = dt.toLocaleString('en-US', { month:'short', day:'numeric', hour:'numeric', minute:'2-digit' });
      const text = String(p.text || '').replace(/[&<>"']/g, (c) => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;','\'':'&#39;'}[c]));
      return `<div class="post"><div class="pmeta"><div class="ptime">${when}</div><button class="pdel" data-pdel="${p.id}" aria-label="Delete"><i data-feather="trash-2"></i></button></div><div class="ptext">${text}</div></div>`;
    }).join('');
    if (window.feather && typeof window.feather.replace === 'function') window.feather.replace();
    qsa('[data-pdel]', list).forEach(btn => {
      btn.addEventListener('click', () => {
        const id = btn.dataset.pdel;
        const next = posts.filter(x => x && x.id !== id);
        writeLS('socialPosts', next);
        renderPosts();
      });
    });
  }
  ...
  function addPost(){
    const ta = byId('postText');
    if (!ta) return;
    const text = ta.value.trim();
    if (!text) return;
    const posts = readLS('socialPosts', []);
    posts.unshift({ id: String(Date.now()) + Math.random().toString(16).slice(2), ts: Date.now(), text });
    writeLS('socialPosts', posts);
    ta.value = '';
    renderPosts();
  }
  ...
  function wire(){
    ...
    const agendaAdd = byId('agendaAdd');
    if (agendaAdd) agendaAdd.addEventListener('click', addAgenda);
    const agendaInput = byId('agendaInput');
    if (agendaInput) agendaInput.addEventListener('keydown', (e) => {
      if (e.key === 'Enter'){
        e.preventDefault();
        addAgenda();
      }
