...
async function animateCaseOpen(results, rollerElement, revealElement) {
    ...
    const sourcePool = caseAnimationPool.length ? caseAnimationPool : results;
    const totalSlots = 42;
    const winnerIndex = totalSlots - 6;

    for (let i = 0; i < totalSlots; i += 1) {
        const isWinner = i === winnerIndex;
        const item = isWinner ? winner : sourcePool[Math.floor(Math.random() * sourcePool.length)];
        track.appendChild(createSlotElement(item, false, isWinner));
    }

    const slots = track.querySelectorAll('.roller-slot');
    const slotWidth = 152;
    const centerOffset = Math.max(0, (rollerElement.clientWidth / 2) - (slotWidth / 2));
    const targetX = -(winnerIndex * slotWidth) + centerOffset;

    const timeline = createTimeline({ easing: 'easeOutExpo' });
    timeline.add({
        targets: track,
...
function createSlotElement(result, isPlaceholder = false, isWinner = false) {
    ...
    slot.className = `roller-slot${isPlaceholder ? ' roller-slot-placeholder' : ''}${isWinner ? ' winner' : ''}`;
