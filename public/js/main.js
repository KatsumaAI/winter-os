...
function renderCommunityUI() {
        ...
        if (state.activeRain) {
            ...
            rainPanel.innerHTML = `
                <div class="community-rain-card">
                    ...
                    <div class="community-rain-entrants">${entrants.length ? entrants.map((entry) => `<span>${escapeHtml(entry.username)}</span>`).join('') : '<span>No entrants yet</span>'}</div>
            ...
            messageList.innerHTML = state.communityMessages.map((message) => `
                <article class="community-message community-message-${escapeHtml(message.type || 'user')}">
                    <div class="community-message-head">
                        <span class="community-message-author">${escapeHtml(message.username || 'KatsuCases')}</span>
                        ${message.is_admin ? '<span class="badge badge-legendary community-message-badge">OWNER</span>' : ''}
    ...
    if (authHint) {
        authHint.innerHTML = state.user
            ? `<span>Logged in as <strong>${escapeHtml(state.user.username)}</strong></span>`
            : '<a href="/signin">Sign in</a> to chat and enter rain.';
    }
    ...
    if (form) {
        form.classList.toggle('disabled', !state.user);
    }
}
...
async function loadCommunityChat(silent = false) {
    try {
        ...
        if (!silent) {
            showToast(error.message || 'Failed to load community chat', 'error');
        }
        return null;
    }
}
